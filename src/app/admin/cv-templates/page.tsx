...
export default function AdminCVTemplatesPage() {
  ...
  // Filtrage local sur les templates
  const filtered = cvTemplates.filter((template) =>
  ...
  const stats = {
    ...
    totalDownloads: cvTemplates.reduce(
      (sum, template) => sum + template.downloads,
      0,
    ),
    mostPopular: cvTemplates.reduce(
      (max, template) => (template.downloads > max.downloads ? template : max),
      { downloads: 0, name: "" },
    ),
  };
  ...
  return (
    <AdminLayout>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-lg font-semibold text-gray-900 dark:text-white">
                  {loading
                    ? "Chargement..."
                    : stats.mostPopular.name || "Aucun"}
                </p>
                <p className="text-sm text-gray-600 dark:text-gray-300">
                  {!loading && `${stats.mostPopular.downloads} téléchargements`}
                </p>
              </div>
            </div>
          </div>
            ...
            <>
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div className="w-32 bg-gray-200 dark:bg-gray-700 rounded-full h-2.5">
                              <div
                                ...
                                style={{
                                  width: `${Math.min(100, (template.downloads / Math.max(1, stats.mostPopular.downloads)) * 100)}%`,
                                }}
                              ></div>
                            </div>
                          </div>
              ...
              <div className="flex flex-col sm:flex-row items-center justify-between mt-6">
                <div className="text-sm text-gray-600 dark:text-gray-400 mb-4 sm:mb-0">
                  {filtered.length} template(s) • Page {page} sur {totalPages}
                </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6 max-w-md w-full mx-4">
            ...
            <div className="flex justify-end space-x-3">
              ...
              <button
                ...
                onClick={() => {
                  deleteCVTemplate(templateToDelete.id);
      ...
      <EntityModal
        ...
        onClose={closeModal}
        onSubmit={(data) => {
          if (editingCVTemplate) {
            editCVTemplate({ ...editingCVTemplate, ...data });
          } else {
            const { name, downloads } = data as {
              name: string;
              downloads: number;
            };
            addCVTemplate({ name, downloads });
          }
        }}
        fields={cvTemplateFields}
        initialData={
          editingCVTemplate
            ? {
                name: editingCVTemplate.name,
                downloads: editingCVTemplate.downloads,
              }
            : undefined
        }
        title={
          editingCVTemplate ? "Modifier le template" : "Ajouter un template"
        }
      />
