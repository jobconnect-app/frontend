...
export default function AdminFavoritesPage() {
  ...
  const favoriteFields = [
    { name: "user", label: "Utilisateur", required: true },
    { name: "job", label: "Offre", required: true },
  ];
  ...
  // Filtrage local sur les favoris
  const filtered = favorites.filter(
    (fav) =>
      fav.user.toLowerCase().includes(search.toLowerCase()) ||
      fav.job.toLowerCase().includes(search.toLowerCase()),
  );
  ...
  const stats = {
    ...
    topUser: favorites.reduce(
      (acc, fav) => {
        const userCount = favorites.filter((f) => f.user === fav.user).length;
        return userCount > acc.count
          ? { user: fav.user, count: userCount }
          : acc;
      },
      { user: "", count: 0 },
    ),
    topJob: favorites.reduce(
      (acc, fav) => {
        const jobCount = favorites.filter((f) => f.job === fav.job).length;
        return jobCount > acc.count ? { job: fav.job, count: jobCount } : acc;
      },
      { job: "", count: 0 },
    ),
  };
  ...
  return (
    <AdminLayout>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-lg font-semibold text-gray-900 dark:text-white truncate">
                  {loading ? "Chargement..." : stats.topUser.user || "Aucun"}
                </p>
                <p className="text-sm text-gray-600 dark:text-gray-300">
                  {!loading && `${stats.topUser.count} favoris`}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-lg font-semibold text-gray-900 dark:text-white truncate">
                  {loading ? "Chargement..." : stats.topJob.job || "Aucune"}
                </p>
                <p className="text-sm text-gray-600 dark:text-gray-300">
                  {!loading && `${stats.topJob.count} favoris`}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col md:flex-row gap-4">
            <div className="flex-1">
              ...
              <div className="relative">
                ...
                <input
                  type="text"
                  placeholder="Utilisateur ou offre..."
            ...
            <>
              ...
              <div className="flex flex-col sm:flex-row items-center justify-between mt-6">
                <div className="text-sm text-gray-600 dark:text-gray-400 mb-4 sm:mb-0">
                  {filtered.length} favori(s) • Page {page} sur {totalPages}
                </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6 max-w-md w-full mx-4">
            ...
            <p className="mb-6 text-gray-700 dark:text-gray-300">
              Voulez-vous vraiment supprimer le favori de{" "}
              <span className="font-semibold">{favoriteToDelete.user}</span> ?
            </p>
      ...
      <EntityModal
        ...
        onSubmit={(data) => {
          if (editingFavorite) {
            editFavorite({ ...editingFavorite, ...data });
          } else {
            const { user, job } = data as { user: string; job: string };
            addFavorite({ user, job });
          }
        }}
        ...
        initialData={
          editingFavorite
            ? {
                user: editingFavorite.user,
                job: editingFavorite.job,
              }
            : undefined
        }
        title={editingFavorite ? "Modifier le favori" : "Ajouter un favori"}
      />
